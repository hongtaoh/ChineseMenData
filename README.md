...
收入分布基于中国劳动力市场的经验数据，结合以下因素模拟生成：

1. **年龄**：不同年龄段的收入呈现阶段性变化，例如刚参加工作时收入较低，中年达到高峰，退休后普遍下降。
2. **教育水平**：教育水平显著影响收入分布，高学历人群的收入普遍高于低学历人群。
...
1. **基准分布**：
   - 根据学历设定基础收入分布。例如：
     - **研究生及以上**：高收入比例最高，低收入比例最低。
     - **高中及以下**：以低收入为主，高收入比例极低。

2. **年龄修正**：
   - 收入分布根据年龄阶段的不同进行修正：
     - **青年阶段（<25岁）**：收入以低档为主。
     - **壮年阶段（25-45岁）**：中高收入档次比例逐步增加。
     - **中年阶段（45-60岁）**：收入高峰期，高收入档次比例达到最大。
     - **退休阶段（>60岁）**：收入以退休金为主，中高收入比例明显降低。

3. **条件概率计算**：
   - 收入的条件概率由学历基准分布和年龄修正系数共同决定。
   - 每个年龄和学历组合的概率归一化，确保总和为 1。

4. **随机采样**：
   - 使用上述条件概率生成目标样本数量的收入档次数据。
